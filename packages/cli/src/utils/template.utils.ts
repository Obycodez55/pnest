...
export async function getAvailableTemplates(): Promise<Template[]> {
    try {
      ...
      for (const dir of templateDirs) {
        const metadataPath = path.join(TEMPLATES_PATH, dir, 'metadata.json');
...
export async function createProjectFromTemplate(
    ...
    const templateDir = path.join(TEMPLATES_PATH, template.name, 'template');
    ...
    await processTemplateFiles(outputDir, {
      projectName,
