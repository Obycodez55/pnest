...
import { PluginManifest, PNestPlugin } from "../commands/plugins/plugin.interface";
import fs from 'fs-extra';
import chalk from "chalk";
import execa from "execa";
import { isNestProject } from "./nest-cli.utils";
 ...
 */
export async function installPlugin(plugin: PluginManifest, provider: string): Promise<void> {
  ...
  try {
    ...
    await updateModuleImports(pluginInstance, provider);
  } catch (error) {
 ...
 */
async function updateModuleImports(plugin: PNestPlugin, provider: string): Promise<void> {
  try {
